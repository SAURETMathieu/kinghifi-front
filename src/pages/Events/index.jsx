/* eslint-disable max-len */
import './index.css';

import { useState, useEffect } from 'react';

function Events() {
//   const [uneVariable, setUneVariable] = useState('');

  return (
    <div>
      <h1 className="event_h1">
        Evénements
      </h1>
      <div className="global">

        <div className="event_container">
          <div className="img_container">
            <img className="event_image" src="images/Barracuda.jpg" alt="Barracuda" />
          </div>
          <div className="text_containeur">
            <h2 className="event_h2">
              Festival de Dour
            </h2>
            <p className="event_type global_description">
              Festival
            </p>
            <p className="event_description global_description">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam neque ut, explicabo similique praesentium tempora exercitationem nesciunt assumenda, cum libero voluptatum quisquam deserunt fuga, pariatur ducimus rem omnis accusamus? Aliquam?
            </p>
            <p className="event_date_debut global_description">
              Lundi 12 février 2024 à 9h00
            </p>
            <p className="event_date_fin global_description">
              Marcredi 14 février 2024 à 23h00
            </p>
            <p className="event_ville global_description">
              Paris
            </p>
            <p className="event_pays global_description">
              France
            </p>
            <p className="event_lieu global_description">
              Scène 28
            </p>
          </div>
        </div>

        <div className="event_container">
          <div className="img_container">
            <img className="event_image" src="images/fete_zik_2014.jpg" alt="fete_zik_2014" />
          </div>
          <div className="text_containeur">
            <h2 className="event_h2">
              Festival de Dour
            </h2>
            <p className="event_type global_description">
              Festival
            </p>
            <p className="event_description global_description">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam neque ut, explicabo similique praesentium tempora exercitationem nesciunt assumenda, cum libero voluptatum quisquam deserunt fuga, pariatur ducimus rem omnis accusamus? Aliquam?
            </p>
            <p className="event_date_debut global_description">
              Lundi 12 février 2024 à 9h00
            </p>
            <p className="event_date_fin global_description">
              Marcredi 14 février 2024 à 23h00
            </p>
            <p className="event_ville global_description">
              Paris
            </p>
            <p className="event_pays global_description">
              France
            </p>
            <p className="event_lieu global_description">
              Scène 28
            </p>
          </div>
        </div>

      </div>

    </div>

  );
}

export default Events;
